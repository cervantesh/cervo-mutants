...
| First-party application repo | Primary adoption target. | `ci-fast` budgeted PR-style run and `nightly` sampled run. |
| First-party shared library | Reusable library target. | Full conservative run if small; otherwise `ci-balanced`. |
| Popular Go repo | External validity check outside first-party code. Prefer `stretchr/testify` or `gin-gonic/gin`. | `ci-fast` and `gremlins-compatible` sampled comparison. |
...
| First-party application core | `./...` | `ci-fast` | 11.51 | 20 | 20 | 20 | 7 | 13 | 0 | 35.00% | 100.00% |
| First-party application core | `./...` | `gremlins-compatible` | 10.16 | 20 | 20 | 20 | 7 | 13 | 0 | 35.00% | 100.00% |
| First-party shared library | `./...` | `ci-fast` | 10.41 | 20 | 20 | 20 | 5 | 15 | 0 | 25.00% | 100.00% |
| First-party shared library | `./...` | `gremlins-compatible` | 5.73 | 20 | 20 | 20 | 5 | 15 | 0 | 25.00% | 100.00% |
...
- `gremlins-compatible` and `ci-fast` intentionally produce the same operator
  ...
  filesystem noise.
- The first-party shared library exposed a Windows junction discovery bug: the
  repository path under the workspace root was a junction, and Go's `WalkDir`
...
- The first-party application core and shared library have low sampled scores.
  That is a test-suite signal,
...
- Add one `nightly` sample for the first-party application core and shared
  library.
...
| First-party application core `./...` | 100 | 12 | 28 | 60 | 30.00% | 40.00% | 50.46s |
| First-party shared library `./...` | 52 | 18 | 10 | 24 | 64.29% | 53.85% | 16.09s |
