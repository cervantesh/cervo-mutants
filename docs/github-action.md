...
- name: Run CervoMutants
  id: cervomut
  uses: cervantesh/cervo-mutants@v0.4.2
  with:
    policy: ci-fast
    budget: 5m
...
- ref normalization accepts standard GitHub tag refs such as
  `refs/tags/v0.4.2` and standard branch refs such as `refs/heads/main`
...
The examples above now pin `@v0.4.2`, which is the latest public release of
